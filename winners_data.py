...
WIN_PORT = int(os.getenv("WIN_PORT"))
...
ENDPOINT = next((ep for ep in ENDPOINTS if 'ckyL' in ep), None)    
...
async def poll_single(client):
    ...
    try:
        ...
        data_list = resp.json().get("appCommunityLuckyList", [])

        if not data_list:
            logger.warning("⚠️ No Winners data returned")
            return
        ...
        latest_data["winners"] = data_list[0]
        
        r.set("winners_data", json.dumps(latest_data["winners"], sort_keys=True))

        # Broadcast immediately
        await broadcast(new_data)
        logger.info(f"{latest_data["winners"]}")
...
async def quic_server():
    ...
    logger.info("🔐 QUIC/WebTransport server running on UDP :%s", WIN_PORT)
