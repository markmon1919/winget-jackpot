...
def fetch_api_data():
    ...
    while not stop_event.is_set():
        try:
            ...
            if current_jackpot != last_jackpot_value:
                prev_signal = state.api_vol_signal  # store previous value
                new_signal = (
            ...
            #     # if state.scatter_mode: break
                ...
                trigger = ((
                    state.rtp and any([
                        ...
                        drain_seconds <= 1, 0.0 < hot_seconds <= 7,
                    ...
                    if state.last_spin is not None:
                        ...
                        # ]):
                            ...
                            if any(
                                ...
                                random.random() < 0.1,
                                predicted_delta_10m > prev_predicted_delta,
                                abs(api_vol_delta) >= 100 or api_vol >= 200 or explosion or session_mode in ["HOT", "DRAIN"]
                            ]):
                                # if state.slow_mode:
                                #     if any([
                                #         # state.hit_win,
            ...
            colored_delta_volume = (
                    f"{colors['BLGRE'] if all([api_vol_delta >= 100, explosion]) else
                       colors['BLRED'] if all([api_vol_delta <= -100, explosion]) else
            ...
            colored_rtp = (
                f"{colors['BLGRE']}⬆{colors['RES']}" if state.rtp else
                f"{colors['BLRED']}⬇{colors['RES']}" if not state.rtp else
                f"{colors['LCYN']}◉{colors['RES']}"
            )
            ...
            message = (
                ...
                f"\n\t{colors['WHTE']}Bet Level{colors['RES']}: {bet_lvl:<17} {colors['WHTE']}Avg Data/Sec{colors['RES']}: {new_data_seconds:<21} {colors['WHTE']}RTP {colors['RES']}({colors['MAG']}{game['name']}{colors['RES']}){colors['RES']}: {colored_rtp}  {colors['WHTE']}Direction{colors['RES']}: {colored_direction} {colors['RES']}"
            ...
            if current_jackpot != last_jackpot_value:
                ...
                if direction is not None:
                    alert_queue.put("tink") if direction == "bullish" else alert_queue.put("ping") if direction == "bearish" else alert_queue.put("trap")
...
def fetch_rtp_data():
    ...
    while not stop_event.is_set():        
        try:
            ...
            if top_name.lower() == game["name"].lower(): state.rtp = top_rtp
